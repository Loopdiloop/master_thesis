...
plt.errorbar(energy_d, strength_d, yerr=strength_err_d[:-1], x_err=None, fmt="D", color=stylesheet.d_color_data, label="(a,d) 188Re")
plt.plot(energy_trans_d[:80], trans_d[:80], color=stylesheet.d_color_inter)

plt.errorbar(energy_t, strength_t, yerr=strength_err_t[:-1], x_err=None, fmt="D", color=stylesheet.t_color_data, label="(a,t) 187Re")
plt.plot(energy_trans_t[:80], trans_t[:80], color=stylesheet.t_color_inter)

#plt.title("gamma-ray SF for both (a,d) and (a,t) with extrapolations.")
#plt.ylabel("$\gamma$SF [1/MeV]",size=18)
