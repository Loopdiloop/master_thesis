{
	...
	TH2F *h = new TH2F("h"," ",10,0.0,   6.556,10,9.083e-10,6.909e-06);
	ifstream strengthfile("strength.nrm");
	float strength[30],strengtherr[30],energyerr[30];
	float energy[291],trans[291];
	int i = 0;
   float a0 =  -0.8875;
   float a1 =   0.2480;
	float x;	
	while(strengthfile){
		...
		else{strengtherr[i-29] = x;}
		i++;
	}
	TGraphErrors *strengthexp = new TGraphErrors(30,energy,strength,energyerr,strengtherr);
    i = 0;
	...
	TLatex t;
	t.SetTextSize(0.05);
	t.DrawLatex(    1.211,2.073e-06,"^{xx}Yy");
	c1->Update();
